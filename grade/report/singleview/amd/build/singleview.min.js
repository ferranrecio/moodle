define("gradereport_singleview/singleview",["exports"],(function(_exports){Object.defineProperty(_exports,"__esModule",{value:!0}),_exports.init=function(){if(initialized)return;initialized=!0,document.body.addEventListener("keydown",keydownHandler),document.querySelectorAll(selectors_override).forEach((input=>{input.addEventListener("change",(()=>{!function(input){const checked=input.checked,[,itemid,userid]=input.getAttribute("name").split("_"),interest="_".concat(itemid,"_").concat(userid);document.querySelectorAll(selectors_input(interest)).forEach((text=>{text.disabled=!checked})),document.querySelectorAll(selectors_select(interest)).forEach((select=>{select.disabled=!checked}))}(input)}))}))};
/**
