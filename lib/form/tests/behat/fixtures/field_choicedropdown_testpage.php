...
$PAGE->set_url('/lib/form/tests/behat/fixtures/field_sample_choicedropdown.php');
...
class test_choice_dropdown extends moodleform {
    ...
    public function definition() {
        ...
        // In behat the choice dropdown element is a regular select input to ensure behat runs fast.
        // However, we want to test also the javascript part so we force the fancy javascript interaction
        // in one of the fields.

        $options = new choicelist();
        $options->set_allow_empty(false);
        $options->add_option('option1', 'Forced text option 1', [
            'description' => 'Description option 1',
            'icon' => new pix_icon('t/groupv', 'core')
        ]);
        $options->add_option('option2', 'Forced text option 2', [
            'description' => 'Description option 2',
            'icon' => new pix_icon('t/groups', 'core')
        ]);

        $mform->addElement('header', 'database', "Test javascript choice dropdown");
        $mform->addElement('button', 'focubutton', "Quick focus button");
        $mform->addElement('choicedropdown', 'example4', "Forced choice dropdown", $options, ['data-force-dropdown' => true]);
