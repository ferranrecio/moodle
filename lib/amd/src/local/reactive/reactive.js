...
export default class {
    ...
    registerComponent(component) {
        ...
        // Handlers that will be executed only once per transaction.
        const delayedHandlers = new Map();
        ...
        handlers.forEach(({watch, handler}) => {
            ...
            let executeHandler = (detail) => {
                // Prevent any watcher from losing the page focus.
                const currentFocus = document.activeElement;
                // Execute watcher.
                handler.apply(component, [detail]);
            ...
            let listener = (event) => {
                if (watch.length > 1) {
                    // Prevent multi state watchers from executing twice.
                    delayedHandlers.set(executeHandler, event.detail);
                } else {
                    executeHandler(event.detail);
                }
            };

            watch.forEach(stateChange => {
                // Save the listener information in case the component must be unregistered later.
                listeners.push({target: this.target, stateChange, listener});
                ...
                // reactive instances.
                this.target.addEventListener(stateChange, listener);
            });

        });

        // All components have an implicit transaction end watcher to execute delayed handlers.
        const transactionWatch = 'transaction:end';
        const executeDelayed = () => {
            const handlersToExecute = new Map(delayedHandlers);
            delayedHandlers.clear();
            handlersToExecute.forEach((detail, executeHandler) => {
                executeHandler(detail);
            });
        };
        listeners.push({target: this.target, transactionWatch, executeDelayed});
        this.target.addEventListener(transactionWatch, executeDelayed);
