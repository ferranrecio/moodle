...
export default class {
     ...
     *
     * Any component interacts with a single main DOM element. For backward compatibility reasons,
     * the element can be provided directly using the descriptor.element or by a query selector string
     * using descriptor.selector. However, newer types of compoments (like the TemplateComponent)
     * requires descriptor.selector.
     *
     * Internally, the component can uses other selectors to select elements within
     * this main element (descriptor.selectors). By default each component will provide it's own
     * default selectors, but those can be overridden by the "descriptor.selectors"
     ...
     * @property {HTMLElement} [selector] the main component selector
     * @property {HTMLElement} [element] all components needs an element to anchor events
    ...
    constructor(descriptor) {
        ...
        if (descriptor.selector !== undefined && descriptor.element === undefined) {
            descriptor.element = document.querySelector(descriptor.selector);
        }
        ...
        this.selector = descriptor.selector ?? null;
        this.element = descriptor.element;

        // The component hash allow to optimize DOM refrehes.
        if (this.element.dataset.mdlComponentHash === undefined) {
            this.element.dataset.mdlComponentHash = Date.now().toString(36);
        }
        this.componentHash = this.element.dataset.mdlComponentHash;
    ...
    /**
     * Get a inner element with a specific data-mdl-ref attribute.
     *
     * This method will only search inside the main element of the component
     * and will ignore any element inside subcomponents.
     *
     * @param {String} reference the reference name.
     * @returns {HTMLElement|null}
     */
    getReference(reference) {
        const elementCandidates = this.element.querySelectorAll('[data-mdl-ref="' + reference + '"]');
        if (elementCandidates.length === 0) {
            return null;
        }
        for (const element of elementCandidates) {
            if (element.closest('[data-mdl-component-hash]') === this.element) {
                return element;
            }
        }
        return null;
    ...
    /**
     * Returns the element selector of the component, if any.
     *
     * @returns {HTMLElement|null}
     */
    getMainSelector() {
