...
export default class extends BaseComponent {
    ...
    _addEventTotalMovement(event) {
        ...
        event.totalMovementX = event.pageX - dragStartPoint.pageX;
        event.totalMovementY = event.pageY - dragStartPoint.pageY;
        ...
        event.newTop = current.top + window.scrollY + event.totalMovementY;
        event.newLeft = current.left + window.scrollX + event.totalMovementX;
        // The affected region possible new top.
        if (this.fullregion !== undefined) {
            const current = this.fullregion.getBoundingClientRect();
            event.newRegionTop = current.top + window.scrollY + event.totalMovementY;
            event.newRegionLeft = current.left + window.scrollX + event.totalMovementX;
        }
    }
