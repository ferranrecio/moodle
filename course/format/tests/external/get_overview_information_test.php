...
final class get_overview_information_test extends \externallib_advanced_testcase {
    /**
     * Test the behaviour of get_overview_information::execute().
     *
     * @covers ::execute
     */
    public function test_get_overview_information(): void {
        ...
        $tabledata = $overviewtable->export_for_external($renderer);
        ...
        foreach ($tabledata->headers as $header) {
            ...
            $this->assertEquals($header->textalign, $resultelement['textalign']);
        }

        // The test should include only the two assign activities.
        $this->assertCount(2, $result['activities']);
        $this->assertEquals(count($tabledata->overviews), count($result['activities']));

        // Validate activity 1.
        $activity = $this->find_by_attribute($result['activities'], 'cmid', (int) $mod1->id);
        $activitytable = $this->find_by_attribute($tabledata->overviews, 'cmid', $mod1->id);
        ...
        foreach ($activitytable->items as $item) {
            ...
            $this->assertEquals(json_encode($item->contentdata), $resultelement['contentjson']);
            $this->assertEquals(json_encode($item->extradata), $resultelement['extrajson']);
        }

        // Validate activity 2.
        $activity = $this->find_by_attribute($result['activities'], 'cmid', (int) $mod2->id);
        $activitytable = $this->find_by_attribute($tabledata->overviews, 'cmid', $mod2->id);
