 ...
 */
class cmactions_test extends \advanced_testcase {
    /**
    ...
    /**
     * Test renaming an activiyt also renames the grade items.
     *
     * @covers ::rename
     */
    public function test_rename_grade_items(): void {
        global $DB;
        $this->resetAfterTest();
        ...
        $gradeitem = $DB->get_record('grade_items', ['iteminstance' => $activity->id]);
        $this->assertEquals('Old name', $gradeitem->itemname);
        ...
        $gradeitem = $DB->get_record('grade_items', ['iteminstance' => $activity->id]);
        $this->assertEquals('New name', $gradeitem->itemname);
    }
