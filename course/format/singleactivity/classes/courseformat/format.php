...
namespace format_singleactivity\courseformat;
...
class format extends format_base {
    ...
    public function extend_course_navigation($navigation, navigation_node $node) {
        ...
        if (has_capability('moodle/course:viewhiddensections', $context)) {
            ...
            if (!empty($modinfo->sections[1])) {
                ...
                $orphanednode = $node->add(
                    get_string('orphaned', 'format_singleactivity'),
                    $this->get_view_url(1),
    ...
    public function create_edit_form_elements(&$mform, $forsection = false) {
        ...
        if (
            !$forsection && ($course = $PAGE->course) && !empty($course->format) &&
            ...
            $element = $mform->addElement(
                ...
                html_writer::tag(
                    'span',
                    get_string('warningchangeformat', 'format_singleactivity'),
                    array('class' => 'error')
                )
            );
    ...
    public function page_set_course(moodle_page $page) {
        ...
        if (
            ...
            if (!empty($cursection) && has_capability(
            ...
            if ($cm === null) {
                if ($this->can_add_activity()) {
                    ...
                    $url = new moodle_url('/course/mod.php', array(
                        'id' => $this->courseid,
    ...
    public function page_set_cm(moodle_page $page) {
        ...
        if (
            $PAGE == $page && ($cm = $this->get_activity()) &&
            $cm->uservisible &&
            ($cm->id === $page->cm->id) &&
