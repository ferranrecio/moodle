...
class course_get_state_test extends \externallib_advanced_testcase {
     ...
     * @covers ::execute
     *
     * @param bool $isadmin Wheter the test should be run as admin or not.
     * @param string $format Course format
     */
    public function test_get_state(bool $isadmin, string $format = 'topics'): void {
        $this->resetAfterTest();
        ...
        $this->create_activity($course->id, 'page', 1, true, $isadmin);
        $this->create_activity($course->id, 'forum', 1, true, $isadmin);
        $this->create_activity($course->id, 'book', 1, false, $isadmin);
        $this->create_activity($course->id, 'assign', 2, false, $isadmin);
        $this->create_activity($course->id, 'glossary', 4, true, $isadmin);
        $this->create_activity($course->id, 'label', 5, false, $isadmin);
        $this->create_activity($course->id, 'feedback', 5, true, $isadmin);
    ...
    public function get_state_provider(): array {
        return [
            'Admin user with singleactivity format' => ['isadmin' => true, 'format' => 'singleactivity'],
            'Admin user with social format' => ['isadmin' => true, 'format' => 'social'],
            'Admin user with topics format' => ['isadmin' => true, 'format' => 'topics'],
            'Admin user with weeks format' => ['isadmin' => true, 'format' => 'weeks'],
            'Admin user with theunittest format' => ['isadmin' => true, 'format' => 'theunittest'],
            'Student user with singleactivity format' => ['isadmin' => false, 'format' => 'singleactivity'],
            'Student user with social format' => ['isadmin' => false, 'format' => 'social'],
            'Student user with topics format' => ['isadmin' => false, 'format' => 'topics'],
            'Student user with weeks format' => ['isadmin' => false, 'format' => 'weeks'],
            'Student user with theunittest format' => ['isadmin' => false, 'format' => 'theunittest'],
        ];
    }
     ...
     * @param bool $isadmin Whether the activity will be created or not by admin user.
     */
    private function create_activity(int $courseid, string $type, int $section, bool $visible = true, bool $isadmin = true): void {
