...
class course_edit_test extends \externallib_advanced_testcase {
    ...
    public function test_execute_course_state(
        ...
        $results = json_decode(course_edit::execute($action, $course->id, [$activity->cmid]));
    ...
    public function test_execute_wrong_courseid() {
        ...
        // Execute course action.
        $results = json_decode(course_edit::execute('cm_state', $course->id + 1, [$activity->cmid]));
    }
    ...
    public function test_execute_target_params() {
        ...
        $results = json_decode(course_edit::execute('targetsection_test', $course->id, [], $section->id));
        $this->assertDebuggingCalled();
        ...
        $results = json_decode(course_edit::execute('targetcm_test', $course->id, [], null, $activity->cmid));
        $this->assertDebuggingCalled();
        $this->assertCount(1, $results);
