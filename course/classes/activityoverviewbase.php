<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

namespace core_course;

use cm_info;
use core\context\module as module_context;
use core_completion\cm_completion_details;
use core_course\local\overview\overviewitem;
use core_course\local\overview\resourceoverview;
use core_course\output\local\overview\activityname;

/**
 * Base class for activity overview.
 *
 * Plugins must extend this class on their \mod_PLUGINNAME\course\overview
 * integration.
 *
 * @package    core_course
 * @copyright  2024 Ferran Recio <ferran@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class activityoverviewbase {
    /** @var cm_info The course module. */
    protected module_context $context;
    /** @var \stdClass $course */
    protected \stdClass $course;

    /**
     * Creates an instance of the appropriate overview class for the given course module.
     *
     * This method attempts to create an instance of a class named `\mod_{modname}\course\overview`,
     * where `{modname}` is the name of the module associated with the given course module.
     * If such a class exists and it extends the current class, an instance of it is returned.
     * Otherwise, an instance of `resourceoverview` is returned.
     *
     * @param cm_info $cm The course module information.
     * @return static An instance of the appropriate overview class.
     * @throws \coding_exception If the found class does not extend the current class.
     */
    public static function instance_from_cm(cm_info $cm): static {
        $classname = "\\mod_{$cm->modname}\\course\\overview";
        if (class_exists($classname)) {
            $result = new $classname($cm);
            if (!($result instanceof static)) {
                throw new \coding_exception("Class $classname must extend " . static::class);
            }
            return $result;
        }
        return new resourceoverview($cm);
    }

    /**
     * Activity Overview Base class constructor.
     *
     * @param cm_info $cm The course module.
     */
    protected function __construct(
        /** @var cm_info The course module. */
        protected cm_info $cm,
    ){
        $this->context = $cm->context;
        $this->course = $cm->get_course();
    }

    /**
     * Get the plugin specific overview items for the activity.
     *
     * Plugins can override this method to provide their own overview items.
     *
     * The resulting array must be indexed by item shortname.
     *
     * @param \renderer_base $output Renderer base.
     * @return overviewitem[] Array of overview items indexed by item shortname.
     */
    public function get_extra_overview_items(\renderer_base $output): array {
        return [];
    }

    /**
     * Get the name of the activity.
     *
     * @param \renderer_base $output Renderer base.
     */
    public final function get_name_overview(\renderer_base $output): overviewitem {
        return new overviewitem(
            name: get_string('name'),
            value: $this->cm->name,
            content: new activityname($this->cm),
        );
    }

    /**
     * Retrieves the due date overview for the activity.
     *
     * @param \renderer_base $output The renderer base instance used for output.
     * @return overviewitem|null null if module does not have a due date.
     */
    public function get_due_date_overview(\renderer_base $output): ?overviewitem {
        return null;
    }

    /**
     * Retrieves the actions overview for the activity.
     * @param \renderer_base $output
     * @return overviewitem|null null if module does not have a main action item.
     */
    public function get_actions_overview(\renderer_base $output): ?overviewitem {
        return null;
    }

    /**
     * Retrieves the completion overview for the activity.
     *
     * @param \renderer_base $output The renderer base instance used for output.
     * @return overviewitem|null null if completion is not enabled.
     */
    public function get_completion_overview(\renderer_base $output): ?overviewitem {
        global $USER;

        $showcompletionconditions = $this->course->showcompletionconditions == COMPLETION_SHOW_CONDITIONS;

        $completiondetails = cm_completion_details::get_instance(
            $this->cm,
            $USER->id,
            $showcompletionconditions
        );

        if (!$completiondetails->is_tracked_user()) {
            return null;
        }

        $showcompletioninfo = $completiondetails->has_completion() &&
            ($showcompletionconditions || $completiondetails->show_manual_completion());

        if (!$showcompletioninfo) {
            return new overviewitem(
                name: get_string('completion_status', 'completion'),
                value: null,
                content: '-',
            );
        }

        $status = $completiondetails->get_overall_completion();

        $completiontype = match ($status) {
            COMPLETION_INCOMPLETE => 'n',
            COMPLETION_COMPLETE => 'y',
            COMPLETION_COMPLETE_PASS => 'pass',
            COMPLETION_COMPLETE_FAIL => 'fail',
        };
        $describe = get_string('completion-' . $completiontype, 'completion');

        return new overviewitem(
            name: get_string('completion_status', 'completion'),
            value: $status,
            content: $describe,
        );
    }
}
