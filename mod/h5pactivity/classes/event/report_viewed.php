...
class report_viewed extends \core\event\base {
    ...
    protected function get_legacy_logdata() {
        $url = 'report.php?';
        if (!empty($this->other['a'])) {
            $url .= 'a='.$this->other['a'];
        }
        if (!empty($this->other['userid'])) {
            $url .= '&userid='.$this->other['userid'];
        }
        if (!empty($this->other['attemptid'])) {
            $url .= '&attemptid='.$this->other['attemptid'];
        }
        return [$this->courseid, 'h5pactivity', 'report viewed', $url, 0, $this->contextinstanceid];
    }
    ...
    protected function validate_data() {
        ...
        if (empty($this->other['a'])) {
            throw new \coding_exception('The \'a\' value must be set in other.');
        }
    }
