...
class reportresults implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $a = new output_attempt($this->attempt, $this->user, $this->courseid);

        $data = (object)[
            'attempt' => $a->export_for_template($output),
        ];

        $results = $this->attempt->get_results();
        $data->results = [];
        foreach ($results as $key => $result) {
            $r = output_result::create_from_record($result);
            $data->results[] = $r->export_for_template($output);
        }
