...
function quiz_num_attempt_summary($quiz, $cm, $returnzero = false, $currentgroup = 0) {
    global $USER;
    $numattempts = quiz_num_attempts($quiz, $cm, $currentgroup);
 ...
 */
function quiz_num_attempts(stdClass $quiz, cm_info|stdClass $cm, int $currentgroup = 0): stdClass {
    global $DB, $USER;
    $numattempts = new stdClass();
    $numattempts->total = $DB->count_records('quiz_attempts', ['quiz' => $quiz->id, 'preview' => 0]);
    if ($numattempts->total) {
        if ($currentgroup) {
            $numattempts->group = $DB->count_records_sql('SELECT COUNT(DISTINCT qa.id) FROM ' .
 ...
 * @return int
 */
function quiz_num_users_who_attempted(int $quizid, int $cmid): int {
    global $DB;
    $context = context_module::instance($cmid);
    $studentsjoins = get_enrolled_with_capabilities_join($context, '', ['mod/quiz:attempt', 'mod/quiz:reviewmyattempts']);
    $params = array_merge(['quiz' => $quizid, 'preview' => 0], $studentsjoins->params);
 ...
 * @return int
 */
function quiz_num_users_who_can_attempt(int $cmid): int {
    global $DB;
    ...
    return $DB->count_records_sql($sql, $studentsjoins->params);
}
