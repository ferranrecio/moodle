...
class overview extends \core_course\activityoverviewbase {
    ...
    private function get_extra_submissions_overview(): ?overviewitem {
        ...
        $groups = $this->get_overview_groups_if_any();
        if (!empty($groups)) {
            $submissions = 0;
            $total = 0;
            foreach ($groups as $groupid) {
                $submissions += $this->workshop->count_submissions(groupid: $groupid);
                $total += $this->workshop->count_participants(groupid: $groupid);
            }
        } else {
    ...
    private function get_extra_assessments_overview(): ?overviewitem {
        ...
        if (!empty($groups)) {
            ...
            $total = 0;
            foreach ($groups as $groupid) {
                $assessments += $this->workshop->count_assessments(true, $groupid);
                $total += $this->workshop->count_assessments(false, $groupid);
            }
        } else {
    ...
    /**
     * Return the groups the user can access, or null if the user can access all groups.
     *
     * This method encapsulates the group mode logic for the current user.
     *
     * @return int[]|null The group ids the user can access, or null if the user can access all groups.
     */
    private function get_overview_groups_if_any(): ?array {
        global $CFG;
        ...
        if ($groupmode == SEPARATEGROUPS) {
            ...
            return array_keys($allowedgroups);
        }
