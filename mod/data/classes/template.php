...
class template {
     ...
     * @param string $templatestr the template string to use
     * @param array $options an array of extra diplay options
     */
    public function __construct(manager $manager, string $templatestr, array $options = []) {
        $this->manager = $manager;
        $this->instance = $manager->get_instance();
        $this->templatestr = $templatestr;

        $context = $manager->get_context();
        $this->manageentries = has_capability('mod/data:manageentries', $context);
        $this->icons = $this->get_icons();
        $this->add_options($options);
        $this->load_template_tags($templatestr);
    }
     ...
     *
     * @param string $templatestr the current template
     */
    protected function load_template_tags(string $templatestr) {
        // Detect action tags.
        $pattern = '/##(?P<tags>\w+?)##/';
        $matches = [];
        preg_match_all($pattern, $templatestr, $matches);
    ...
    private function parse_entry(stdClass $entry): string {
        ...
        $result = str_ireplace($patterns, $replacement, $this->templatestr);
    ...
    private function get_fields_replacements(stdClass $entry): array {
        ...
        foreach ($fields as $field) {
            ...
            $result[$pattern] = highlight(
                $this->search,
                $field->display_browse_field($entry->id, $this->templatestr)
            );
            // Field id.
    ...
    private function post_parse(string $result, stdClass $entry): string {
        ...
        if ($this->forcecomments && strpos($this->templatestr, '##comments##') === false) {
    ...
    protected function get_tag_more_replacement(stdClass $entry, bool $canmanageentry): string {
        ...
        $url = new moodle_url($this->baseurl, [
            'rid' => $entry->id,
            'filter' => 1, // Revisar si es pot fer.
        ]);
        return html_writer::tag(
            'span',
