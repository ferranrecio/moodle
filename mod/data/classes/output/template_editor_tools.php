...
class template_editor_tools implements templatable, renderable {
    ...
    protected function get_other_tags(string $templatename = null): array {
        ...
        if (core_tag_tag::is_enabled('mod_data', 'data_records')) {
            $taglist['##tags##'] = get_string('tags') . ' - ##tags##';
        }
        if ($templatename == 'addtemplate') {
            return $this->get_optgroup_data($name, $taglist);
        }
        $taglist['##timeadded##'] = get_string('timeadded', 'data') . ' - ##timeadded##';
        $taglist['##timemodified##'] = get_string('timemodified', 'data') . ' - ##timemodified##';
        $taglist['##user##'] = get_string('user') . ' - ##user##';
        $taglist['##userpicture##'] = get_string('userpic') . ' - ##userpicture##';
        $taglist['##approvalstatus##'] = get_string('approvalstatus', 'data') . ' - ##approvalstatus##';
        $taglist['##id##'] = get_string('id', 'data') . ' - ##id##';

        if ($templatename == 'singletemplate') {
            return $this->get_optgroup_data($name, $taglist);
        }

        $taglist['##comments##'] = get_string('comments', 'data') . ' - ##comments##';
